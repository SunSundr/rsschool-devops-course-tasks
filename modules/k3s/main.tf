...
resource "aws_instance" "k3s_worker" {
  ...
  user_data = <<-EOF
              ...
              until curl -k https://${aws_instance.k3s_master.private_ip}:6443/ping >/dev/null 2>&1; do
                sleep 10
              done
              
              # Get node token from master
              until NODE_TOKEN=$(ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 ec2-user@${aws_instance.k3s_master.private_ip} 'cat /home/ec2-user/node-token' 2>/dev/null); do
                sleep 10
              done
